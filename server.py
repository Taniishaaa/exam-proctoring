...
# ---------------- MCQ additions  ----------------
EXAM_DURATION = 30 # 5 minutes (seconds) for MCQ/exam
...
def _ensure_excel_sheet():
    """
    Ensure results.xlsx exists, is readable, and has the standard header:
      ["Roll","Name","Flag","MCQ","ISA"]
    Returns (wb, ws) for further updates.
    If existing file is unreadable/corrupt, recreate a fresh workbook.
    """
    try:
        if not excel_path.exists():
            wb = Workbook()
            ws = wb.active
            ws.append(["Roll", "Name", "Flag", "MCQ", "ISA"])
            wb.save(excel_path)
        wb = load_workbook(excel_path)
        ws = wb.active
        # Check header
        header = [c for c in next(ws.iter_rows(min_row=1, max_row=1, values_only=True))]
        expected = ["Roll", "Name", "Flag", "MCQ", "ISA"]
        if header[:len(expected)] != expected:
            logger.warning("[Server] results.xlsx header mismatch — resetting header.")
            wb = Workbook()
            ws = wb.active
            ws.append(expected)
            wb.save(excel_path)
        ...
        logger.warning(f"[Server] Excel missing/corrupt — recreating: {e}")
        wb = Workbook()
        ws = wb.active
        ws.append(["Roll", "Name", "Flag", "MCQ", "ISA"])
    return wb, ws
...
def start_mcq():
    ...
    global exam_started, exam_finished  
    with mcq_lock:
        mcq_active = True
        ...
        mcq_final_scores = {}
        exam_started = True        # ✅ flip flag for students
        exam_finished = False
...
def get_exam_status():
    global exam_started, exam_finished
    return {"started": exam_started, "finished": exam_finished}
...
def submit_mcq_answer(roll: str, qnum: int, choice: int):
    """
    Record a student's answer for a given question.
    Stores answers into mcq_student_answers[roll][qnum] = choice
    """
    roll = str(roll)
    try:
        if roll not in mcq_student_answers:
            mcq_student_answers[roll] = {}
        mcq_student_answers[roll][int(qnum)] = int(choice)
        logger.info(f"[Server] Recorded answer roll={roll} q={qnum} ans={choice}")
        return True
    except Exception as e:
        logger.error(f"[Server] ERROR submit_mcq_answer roll={roll} q={qnum}: {e}")
        return False
...
def _finalize_and_record(roll: str):
    """
    Finalize MCQ answers for a student, compute marks considering warnings,
    update Teacher, and write to Excel with columns:
      Roll | Name | Flag | MCQ | ISA
    Rows are kept sorted by Roll.
    """
    roll = str(roll)
    answers = mcq_student_answers.get(roll, {})
    raw = 0

    # compute raw score using MCQ_QUESTIONS (each correct = 10 marks)
    for qnum, ans in answers.items():
        try:
            qnum_i = int(qnum)
            if qnum_i in MCQ_QUESTIONS and MCQ_QUESTIONS[qnum_i].get("answer") == int(ans):
                raw += 10
        except Exception as e:
            logger.warning(f"[Server] WARN scoring roll={roll} q={qnum}: {e}")

    # apply flag logic
    flags = int(student_flags.get(roll, 0))
    if flags >= 2:
        final = 0
    elif flags == 1:
        final = int(raw * 0.8)
    ...
    logger.info(f"[Server] Finalized MCQ roll={roll} raw={raw} flags={flags} final={final}")
    ...
    # update Teacher
    try:
        teacher_proxy.update_mcq_marks(roll, final, raw)
    except Exception as e:
        logger.warning(f"[Server] WARN teacher.update_mcq_marks: {e}")

    # write/update Excel
    try:
        wb, ws = _ensure_excel_sheet()

        found = False
        for row in ws.iter_rows(min_row=2):
            if str(row[0].value) == roll:
                ws.cell(row=row[0].row, column=3, value=flags)   # Flag
                ws.cell(row=row[0].row, column=4, value=final)   # MCQ
                found = True
                break

        if not found:
            ws.append([int(roll),
                       roll_to_name.get(roll, f"Student{roll}"),
                       flags,
                       final,
                       "NA"])
        ...
        logger.error(f"[Server] Excel write failed for roll={roll}: {e}")
...
def accept_backup_result(roll, final):
    """Accept MCQ result from backup server and write into Excel."""
    roll = str(roll)
    try:
        teacher_proxy.update_mcq_marks(roll, final, final)
    except Exception as e:
        logger.warning(f"[Server] WARN teacher.update_mcq_marks (from backup): {e}")
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2):
            if str(row[0].value) == roll:
                ws.cell(row=row[0].row, column=4, value=int(final))  # MCQ
                updated = True
                break
        if not updated:
            ws.append([int(roll),
                       roll_to_name.get(roll, f"Student{roll}"),
                       int(student_flags.get(roll, 0)),
                       int(final),
                       "NA"])
        wb.save(excel_path)
    except Exception as e:
        logger.warning(f"[Server] WARN writing MCQ result to excel (from backup): {e}")
...
def _auto_submit_pending_mcq():
    ...
    """
    with mcq_lock:
        # ✅ include both registered students AND those who already answered
        all_candidates = set(students_registry.keys()) | set(mcq_student_answers.keys())
        pending = [r for r in all_candidates
...
def _do_synchronization_background():
    """Perform Berkeley time synchronization (background)."""
    import datetime, time, sys
    global cvs, local_time
    try:
        logger.info("[Server] Starting Berkeley sync (background)...")

        # 1. Reset global cvs dict
        with cvs_lock:
            cvs = {}
        ...
        server_time_str = local_time.strftime("%H-%M-%S")

        # 3. Ask Teacher for CV
        try:
            teacher_proxy.calculate_cv(server_time_str)
        except Exception as e:
            logger.warning("[Server] WARN teacher.calculate_cv: %s", e)

        # 4. Collect CVs from registered students
        student_times = getattr(sys.modules[__name__], "student_times", {})
        if student_times:
            for roll, st in student_times.items():
                try:
                    if st and local_time:
                        cv = (st - local_time).total_seconds()
                        with cvs_lock:
                            cvs[f"Student {roll}"] = cv
                        logger.info(f"[Server] Received CV from Student {roll}: {cv}")
                except Exception as e:
                    logger.warning(f"[Server] WARN calculating CV for Student {roll}: {e}")
        else:
            logger.warning("[Server] No student times available for CV calculation.")

        # 5. Server always reports its own CV = 0
        with cvs_lock:
            cvs["Server"] = 0.0

        # 6. Wait up to 12s for Teacher CV to arrive
        ...
        # 7. Compute average CV
        with cvs_lock:
            if not cvs:
                logger.warning("[Server] No CVs received at all, skipping sync.")
                return
            avg_cv = sum(cvs.values()) / len(cvs)
        ...
        try:
            ...
            logger.warning("[Server] WARN teacher.apply_adjustment: %s", e)
        ...
        for node, adj in adjustments.items():
            if node.startswith("Student"):
                roll = node.split()[1]
                try:
                    old = student_times.get(roll)
                    if old:
                        newt = old + datetime.timedelta(seconds=adj)
                        student_times[roll] = newt
                        logger.info(f"[{node}] Adjusted local time -> {newt.strftime('%H-%M-%S')}")
                except Exception as e:
                    logger.warning(f"[Server] WARN adjusting clock for {node}: {e}")

        # Server
        if local_time:
            delta = adjustments.get("Server", 0.0)
            local_time += datetime.timedelta(seconds=delta)
            logger.info(f"[Server] Adjusted local time by {delta:.1f}s -> {local_time.strftime('%H-%M-%S')}")

        # 9. Announce phase complete
        ...
        logger.error("[Server] Synchronization error: %s", e, exc_info=True)
...
def cheating_detection(roll):
    ...
    if flag == 1:
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2):
                if str(row[0].value) == roll:
                    ws.cell(row=row[0].row, column=3, value=2)  # Flag
                    ws.cell(row=row[0].row, column=4, value=0)  # MCQ
                    updated = True
                    break
...
def exam_completed():
    ...
    Auto-finalizes submissions for all students who haven't submitted.
    """
    global exam_finished
    exam_finished = True

    logger.info("[Server] Exam completion received. Finalizing MCQ submissions...")

    with students_lock:
        rolls = list(students_registry.keys())

    pending = []
    for roll in rolls:
        try:
            if roll not in terminated_students:
                logger.info(f"[Server] Auto-finalizing roll={roll}")
                _finalize_and_record(roll)
            else:
                logger.info(f"[Server] Skipping terminated student {roll}")
        except Exception as e:
            logger.error(f"[Server] ERROR auto-finalizing for {roll}: {e}")
            pending.append(roll)

    if pending:
        logger.warning(f"[Server] Some auto-finalizations failed: {pending}")

    # Tell teacher results are ready
    try:
...
def run_server():
    ...
    srv.register_function(get_exam_status, "get_exam_status")
...
if __name__ == "__main__":
    logger.info("server.py loaded and logging is active")
    run_server()
